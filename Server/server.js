...
const AdminUsername = "$2b$10$CZZV.J6.NiFJ04A83VFkCuxc8Ohc8ZXgvoMq9tXxF9rNt58sSbgSi";     //Admin
const AdminPassword = "$2b$10$7NivWKUNsXSCWNM2B16NXe16cMcXs/UrEI5K5KrGFtl8iQaEz0sI6";     //12345678
...
io.on('connection', (socket) =>{
    ...
    socket.on("chat message", (msg) => {
        let message = {username: users[socket.id], text: msg, time: moment().format("h:mm a")};
        let sql = "INSERT INTO messages SET ?"
        let query = db.query(sql, message, (err, result) => {
          if(err) throw err;
          socket.broadcast.emit("chat message", {msg: msg, name: users[socket.id], time: moment().format("h:mm a")});
        socket.emit("chat message", {msg: msg, name: "Du", time: moment().format("h:mm a")});
        });
    });
    ...
    socket.on("login-try", async data => {
      if(await bcrypt.compare(data.username, AdminUsername) && await bcrypt.compare(data.password, AdminPassword)){
        socket.emit("access");
    ...
    socket.on("admin-connected", ip => {
      for(let i = 0; i < AdminWhitelist.length; i++){
        if(ip == AdminWhitelist[i]){
          socket.emit("admin-welcome-message", "Willkommen Admin");
        }else{
          socket.emit("admin-verweigert", "Zutritt verweigert, bitte erst anmelden");
          socket.disconnect();
        }
      }
    })
    ...
    socket.on("Befehl", befehl => {
      alleBefehle = ["alle Befehle: ","!help", "!allUsers", "$kick id"];
      if(befehl.charAt(0) == "$"){
        let kickID = befehl.slice(6);
        console.log(kickID)
        io.to(kickID).emit("chat message", {msg: "Du wurdest gekickt", name: "Admin", time: moment().format("h:mm a")});
